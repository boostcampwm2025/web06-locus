...
export function NotificationsTab({
  ...
  return (
    ...
    >
        ...
        <p className="text-gray-500 text-lg">
          ...
          {readOnly && (
            <span className="block mt-1 text-sm text-gray-400">
              (데스크톱에서는 조회만 가능합니다. 모바일 PWA에서 설정을 변경할 수
              있습니다.)
            </span>
          )}
        </p>
      ...
      {notificationLoading ? (
        <div className="p-8 rounded-[40px] border-2 border-gray-50 bg-gray-50/50 animate-pulse">
          <div className="h-14 bg-gray-200 rounded-2xl w-2/3" />
          ...
          >
            <div className="flex items-center justify-between mb-8">
              ...
              <button
                type="button"
                onClick={() =>
                  !disabled && onNotificationToggle(!isNotificationEnabled)
                }
                disabled={disabled}
              ...
              {isNotificationEnabled && (
                ...
                >
                  <div className="pt-8 border-t border-orange-100 space-y-6">
                    ...
                    {/* 알림 시간 설정 (데스크톱에서는 조회만) */}
                    <div
                      className={`flex items-center justify-between bg-gray-50 p-4 rounded-2xl ${disabled ? 'pointer-events-none opacity-80' : ''}`}
                    >
